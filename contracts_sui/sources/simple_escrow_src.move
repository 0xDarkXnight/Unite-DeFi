/// Module: simple_escrow_src
module simple_limit_order_protocol::simple_escrow_src {
    ...
    /// Source escrow for cross-chain atomic swaps (holds user tokens)
    public struct SimpleEscrowSrc<phantom T> has key, store {
        id: UID,
        amount: u64,
    ...
    /// Deposit tokens into escrow (called by user after creation)
    public fun deposit<T>(
        escrow: &mut SimpleEscrowSrc<T>,
        payment: Coin<T>,
        ctx: &mut TxContext
    ) {
        ...
        if (coin::value(&payment) == escrow.amount) {
            ...
            transfer::public_transfer(payment, escrow.user);
        };
    ...
    /// Withdraw tokens with secret (called by resolver)
    ...
    /// Refund tokens to user after timelock
    public fun refund<T>(
        escrow: &mut SimpleEscrowSrc<T>,
        clock: &Clock,
        ctx: &TxContext
    ...
    /// Get escrow status
    public fun get_status<T>(escrow: &SimpleEscrowSrc<T>): (bool, bool, bool, u64) {
    ...
    /// Get escrow details
    public fun get_details<T>(escrow: &SimpleEscrowSrc<T>): (u64, vector<u8>, u64, address, address) {
        (escrow.amount, escrow.secret_hash, escrow.timelock, escrow.resolver, escrow.user)
    }
    
    /// Check if escrow has balance
    public fun has_balance<T>(escrow: &SimpleEscrowSrc<T>): bool {
